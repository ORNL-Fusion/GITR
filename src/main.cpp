...
if (restart==0){
     std::string input_path = "input/";
      ...
      std::string input_path = "input/restart/";
}
else{
     std::cout
         << "ERROR: Specify restart flag as 0 or 1. "
         << std::endl;
   }
...
// Material Surfaces - flux weighted source
 ...
 {
 std::cout << "Read Restart File " << std::endl;
 ...
 std::cout << "Starting restartFile import " << std::endl;
 vector<gitr_precision> xpRestart(nP), ypRestart(nP), zpRestart(nP),vxRestart(nP), vyRestart(nP), vzRestart(nP), chargeRestart(nP), massRestart(nP);

if( restart > 0 )
     {
 libconfig::Config cfg_particles;
 std::string ncParticleRestartFile;
 int nPartfile = 0;
 if (world_rank == 0) {
   getVariable(cfg, "particleRestart.ncFileString", ncParticleRestartFile);
   std::cout << "About to try to open NcFile ncp0 " << std::endl;
   static const int NC_ERR = 2;
   try {
	    netCDF::NcFile ncp0("input/restart/" + ncParticleRestartFile, netCDF::NcFile::read);
    ...
    netCDF::NcFile ncp("input/restart/" + ncParticleRestartFile, netCDF::NcFile::read);
    std::cout << "getting dim nP" << std::endl;
    netCDF::NcDim ps_nP(ncp.getDim("nP"));

    nPartfile = ps_nP.getSize();
    xpRestart.resize(nPartfile);
    ypRestart.resize(nPartfile);
    zpRestart.resize(nPartfile);
    vxRestart.resize(nPartfile);
    vyRestart.resize(nPartfile);
    vzRestart.resize(nPartfile);
    chargeRestart.resize(nPartfile);
    massRestart.resize(nPartfile);
    std::cout << " Restart nPartfile "<< nPartfile << std::endl;
    ...
    ncp_x.getVar(&xpRestart[0]);
    ncp_y.getVar(&ypRestart[0]);
    ncp_z.getVar(&zpRestart[0]);
    ncp_vx.getVar(&vxRestart[0]);
    ncp_vy.getVar(&vyRestart[0]);
    ncp_vz.getVar(&vzRestart[0]);
    ncp_charge.getVar(&chargeRestart[0]);
    std::cout << "defined restart vectors " << std::endl;
    ncp.close();
 }
 std::cout << "particle restart file import done!" << std::endl;
 sim::Array<gitr_precision> pSurfNormX(nP), pSurfNormY(nP), pSurfNormZ(nP), px(nP),
  py(nP), pz(nP), pvx(nP), pvy(nP), pvz(nP), pcharge(nP), pmass(nP);
 int surfIndexMod = 0;
 ...
 for (int i = 0; i < nP; i++) {
    ...
    vy = vyRestart[i];
    vz = vzRestart[i];
    charge = chargeRestart[i];
    amu = massRestart[i];
    particleArray->setParticleV(i, x, y, z, vx, vy, vz, Z, amu, charge,dt);
    // int currentSegment = particleArray->getSegment(i);
