...
std::vector<float> Efieldr(nR_Bfield*nZ_Bfield), Efieldz(nR_Bfield*nZ_Bfield),Efieldt(nR_Bfield*nZ_Bfield),minDist(nR_Bfield*nZ_Bfield);
float thisE[3] = {0.0,0.0,0.0};
//std::cout << "Efiel init commented out" << std::endl;
...
background_amu = new float[nS];
background_flow = new float[nS];
maxDensity = new float[nS];
maxTemp_eV = new float[nS];
...
#endif
#else
        //std::cout << "starting mover " << std::endl;
...
moveTime = moveTime + (moveTime1.wall - moveTime0.wall);
        ...
        //std::cout << "starting geom check " << std::endl;
...
#if USEIONIZATION > 0
       /* std::cout << "starting ionization operator " << std::endl;
        for(int j =1;j<nP;j++)
        {
            std::cout << "pos loop " << hostCudaParticleVector[j].x << " "
                << hostCudaParticleVector[j].x << std::endl;
        }*/
