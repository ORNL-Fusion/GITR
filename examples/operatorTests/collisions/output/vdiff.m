...
for i=1:nT
    ...
    cosXsi = cos(2*pi*xsi);
    sinXsi = sin(2*pi*xsi);
    vColl(:,1) = (parallel_direction(:,1) + sqrt(nu*dt*0.5).*n2'.*(perp_direction1(:,1).*cosXsi' + perp_direction2(:,1).*sinXsi'));
    vColl(:,2) = (parallel_direction(:,2) + sqrt(nu*dt*0.5).*n2'.*(perp_direction1(:,2).*cosXsi' + perp_direction2(:,2).*sinXsi'));
    vColl(:,3) = (parallel_direction(:,3) + sqrt(nu*dt*0.5).*n2'.*(perp_direction1(:,3).*cosXsi' + perp_direction2(:,3).*sinXsi'));
    vCollMag = sqrt(vColl(:,1).^2+vColl(:,2).^2+vColl(:,3).^2);
    vNew = v0./vCollMag.*vColl;
    v_par = vNew;
