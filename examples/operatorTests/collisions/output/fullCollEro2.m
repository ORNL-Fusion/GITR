...
%     vfz = collV(vz,vbz,m1,m2);
    nu_s0 = interp1(Eparticle,nu_s,E,'pchip',0);
    nu_par0 = interp1(Eparticle,nu_par,E,'pchip',0);
    nu_d0 = interp1(Eparticle,nu_d,E,'pchip',0);
    nu_E0 = interp1(Eparticle,nu_E,E,'pchip',0);
    ...
    coeff_par = 1/stepAdjust*n1.*sqrt(dt*abs(nu_E0).*vRel2);
    coeff_par1 = 0*n2.*sqrt(dt*abs(nu_E0).*vRel2);
    coeff_par2 = 0*n3.*sqrt(dt*abs(nu_E0).*vRel2);
    cosXsi = cos(2.0*pi*xsi);
    sinXsi = sin(2.0*pi*xsi);
    coeff_perp1 = 1/stepAdjust*cosXsi.*sqrt(dt*0.5*nu_d0.*vRel2);
    coeff_perp2 = 1/stepAdjust*sinXsi.*sqrt(dt*0.5*nu_d0.*vRel2);
    ...
    vCollx = drift.*d_parx + coeff_par.*d_parx + coeff_perp1.*e1x + coeff_perp2.*e2x+ coeff_par1.*e1x + coeff_par2.*e2x;
    vColly = drift.*d_pary + coeff_par.*d_pary + coeff_perp1.*e1y + coeff_perp2.*e2y+ coeff_par1.*e1y + coeff_par2.*e2y;
    vCollz = drift.*d_parz + coeff_par.*d_parz + coeff_perp1.*e1z + coeff_perp2.*e2z+ coeff_par1.*e1z + coeff_par2.*e2z;
...
vmag = sqrt((vx-flowV0).^2 +vy.*vy +vz.*vz);
